...
describe('isMediumImplemented / loadMedium', () => {
  it('is false for every medium before any manifest.js exists', () => {
    // At this point in the rebuild (end of Phase 2a) no medium manifest has been built yet.
    for (const entry of MEDIUM_META) {
      expect(isMediumImplemented(entry.id)).toBe(false)
    }
  })
  ...
  it('rejects with a clear, catchable error for a known-but-unbuilt medium', async () => {
    await expect(loadMedium('gallery')).rejects.toThrow(/has not been built yet/)
  })
})
