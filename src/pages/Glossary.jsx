...
const GROUPS = [
  ...
  },
  {
    ...
    note: 'Terms used when testing a strategy against history.',
    terms: [
      ['Backtest', 'A simulation of how a strategy would have performed historically, using past prices and past rankings. Past performance in a backtest is not a guarantee of future results.'],
      ['Equal-weight strategy', 'A backtest strategy that puts the same dollar amount into every selected position, regardless of rank or conviction.'],
      ['Rank-weighted strategy', 'A backtest strategy that sizes positions by their research rank, putting more capital into higher-ranked names instead of splitting money evenly.'],
      ['Benchmark', 'The reference return a strategy is measured against — here, the S&P 500 (via SPY) over the identical time window. The relevant question is not "did this go up," but "did this beat the index I could have simply bought instead."'],
      ['Total return', 'The overall percentage gain or loss on invested capital over the full backtest window, including price change (and dividends where modeled).'],
      ['Sharpe ratio', 'Return earned per unit of volatility (risk) taken on. Two strategies can post the same return, but the one with a higher Sharpe ratio got there with a smoother ride.'],
      ['Turnover', 'How frequently a strategy buys and sells relative to the capital deployed. Higher turnover generally means more trades, more transaction costs, and more tax events in real-world use.'],
      ['Win rate', 'The percentage of closed trades in a backtest that were profitable.'],
    ],
  },
]
