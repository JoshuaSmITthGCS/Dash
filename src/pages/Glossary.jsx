...
const GROUPS = [
  ...
  },
  {
    ...
    terms: [
      ...
      ['Realized volatility percentile', 'Where a stock’s current 20-day realized volatility sits (0-100) versus its own trailing one-year range of that same rolling figure – the "volatility cone" technique, applied to realized (not implied) volatility. This is available immediately (no archive to wait on), and was this app’s original stand-in before "IV percentile" existed. Prefer IV percentile once it’s populated for a given ticker; this one remains useful in the meantime and as a cross-check. Backtested against real historical prices (not a synthetic chain – no options data is needed for this one): a high reading here has a strong, statistically overwhelming track record of preceding a FALL in realized volatility over the following month (rank correlation of about -0.48 across a 250-name, multi-year sample) – real evidence behind the premise premium-selling screens implicitly assume. It has no meaningful ability to predict which direction the stock itself moves next, which is expected – it was never meant to. Blank when there isn’t enough trading history to compute it.'],
