...
| Tax-lot ledger | **Exists — first slice built, with sign-off** | `src/lib/taxLots.js` (FIFO cross-lot depletion engine), wired into `usePortfolioForms.js`/`Holdings.jsx`/`HoldingCard.jsx` | Each position document was already lot-shaped (one purchase, one cost basis); the actual gap was a FIFO engine spanning multiple lots of the same ticker, not a new data model. Specific-ID UI and the wash-sale detector remain unbuilt — see `docs/BUILD-PLAN.md` B3 |
| Wash-sale warning engine | Requires new architecture, blocked | none found | Blocked on the tax-lot ledger existing first — lots now exist; this still needs its own build once there's real depletion history to reason about |
