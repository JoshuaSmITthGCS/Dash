"""Collect AFFO-per-share (or Core FFO / FFO-as-adjusted per share) from earnings-release
exhibits for the equity-REIT universe.

Standalone and opt-in, run separately from ``pipeline/fetch_advisor.py`` -- the same pattern
``pipeline/collect_operating_kpis.py`` already uses for a text-extraction source that should not
silently become part of the scheduled refresh's hot path before it has been checked.
...
``pipeline/operating_kpis_reit_affo.extract_affo_per_share`` against it. No new filing discovery.

Scope: ``pipeline/config/operating_kpi_universe_reit_affo.json``'s ``reit_symbols`` list, not the
whole universe -- see that file's own comment for why, and for why mortgage REITs are excluded.

Output: ``pipeline/data/pit/operating_kpis_reit_affo.jsonl``, append-only, one record per
(ticker, accession) actually resolved. Re-running skips accessions already on disk.

    python pipeline/collect_operating_kpis_reit_affo.py --limit 5      # sample
    python pipeline/collect_operating_kpis_reit_affo.py                # whole configured list
    python pipeline/collect_operating_kpis_reit_affo.py --report        # coverage of what's on
                                                                          # disk, fetch nothing

IMPORTANT: this collector, and the extraction pattern it runs, have not been validated against
a broad, live sample of real filings -- see pipeline/operating_kpis_reit_affo.py's module
docstring, pipeline/filing_text.py's module docstring, and docs/LIMITATIONS.md. Run with
...
KPI_STORE_PATH = os.path.join(HERE, "data", "pit", "operating_kpis_reit_affo.jsonl")


def _configured_symbols():
    config = load_json("operating_kpi_universe_reit_affo.json", from_config=True) or {}
    reits = list(config.get("reit_symbols") or ())
    return {symbol: "reit" for symbol in reits}
...
def collect(client, symbols_and_industry, *, path=KPI_STORE_PATH, releases_path=RELEASES_PATH,
    ...
    read -- more than one lets a coverage report distinguish "this REIT has never disclosed
    AFFO/Core-FFO/FFO-as-adjusted in the phrasing this looks for" from "the one release we tried
    happened not to match", without multiplying the SEC request budget by the whole release
    history.
    """
            ...
            for release in releases:
                ...
                if text is None:
                    record = {"ticker": ticker, "cik": cik, "industry_group": industry_group,
                              ...
                              "affo_per_share": None, "status": "exhibit_unavailable"}
                else:
                    value, detail = extract_affo_per_share(text)
                    record = {"ticker": ticker, "cik": cik, "industry_group": industry_group,
                              "accession": release["accession"], "release_date": release["release_date"],
                              "affo_per_share": value, "status": detail["status"],
                              "detail": detail}
...
def report(path=KPI_STORE_PATH):
    ...
    matched_tickers = {row["ticker"] for row in rows if row.get("affo_per_share") is not None}
    status_counts = {}
    for row in rows:
...
def main(argv=None):
    ...
    if not symbols:
        LOG.warn("No symbols configured in operating_kpi_universe_reit_affo.json, nothing to collect")
        return 1
