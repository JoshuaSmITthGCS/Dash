"""Collect monthly/daily active user counts from earnings-release exhibits for media/internet.

Standalone and opt-in, run separately from ``pipeline/fetch_advisor.py`` -- the same pattern
``pipeline/collect_operating_kpis.py`` already uses for a data source that should not silently
become part of the scheduled refresh's hot path before it has been checked.
...
``pipeline/operating_kpis_media.extract_monthly_active_users`` and
``extract_daily_active_users`` against it -- one fetch per release covers both metrics. No new
filing discovery.

Scope: ``pipeline/config/operating_kpi_universe_media.json``'s media/internet symbol list, not the
whole universe -- see that file's own comment for why, including the tickers expected to come
back "not_found" (streaming-subscriber disclosers, broadcasters).

Output: ``pipeline/data/pit/operating_kpis_media.jsonl``, append-only, one record per (ticker,
accession) actually resolved. Re-running skips accessions already on disk.

    python pipeline/collect_operating_kpis_media.py --limit 5      # sample
    python pipeline/collect_operating_kpis_media.py                # whole configured list
    python pipeline/collect_operating_kpis_media.py --report        # coverage of what's on disk, fetch nothing

IMPORTANT: this collector, and the extraction patterns it runs, have not been validated against a
broad, live sample of real filings -- see pipeline/filing_text.py's module docstring and
...
KPI_STORE_PATH = os.path.join(HERE, "data", "pit", "operating_kpis_media.jsonl")


def _configured_symbols():
    config = load_json("operating_kpi_universe_media.json", from_config=True) or {}
    media = list(config.get("media_symbols") or ())
    return {symbol: "media" for symbol in media}
...
def collect(client, symbols_and_industry, *, path=KPI_STORE_PATH, releases_path=RELEASES_PATH,
    ...
    MAU/DAU in the phrasing this looks for" from "the one release we tried happened not to
            ...
            for release in releases:
                ...
                if text is None:
                    record = {"ticker": ticker, "cik": cik, "industry_group": industry_group,
                              ...
                              "monthly_active_users": None, "daily_active_users": None,
                    ...
                    if mau_value is not None or dau_value is not None:
                        status = "matched"
                    elif mau_detail["status"] != "not_found":
                        status = mau_detail["status"]
                    else:
                        status = dau_detail["status"]
                    record = {"ticker": ticker, "cik": cik, "industry_group": industry_group,
                              "accession": release["accession"], "release_date": release["release_date"],
                              "monthly_active_users": mau_value, "daily_active_users": dau_value,
                              "status": status,
                              "detail": {"monthly_active_users": mau_detail, "daily_active_users": dau_detail}}
                    if mau_value is not None or dau_value is not None:
                        matched += 1
...
def report(path=KPI_STORE_PATH):
    """Coverage of what's on disk -- how much of the configured universe resolved a value (either
    metric counts as a match, since not every filer reports both), and the breakdown of why the
    rest did not, without fetching anything new.
    """
    rows = []
    try:
        with open(path, encoding="utf-8") as handle:
    ...
    matched_tickers = {row["ticker"] for row in rows
                       if row.get("monthly_active_users") is not None
                       or row.get("daily_active_users") is not None}
    status_counts = {}
    for row in rows:
    ...
    return {
        ...
        "note": ("Coverage of ATTEMPTED symbols only, not the full configured list, until "
                 "`collect` has been run for all of them. Not every configured ticker is expected "
                 "to report MAU/DAU -- streaming-subscriber disclosers and broadcasters correctly "
                 "return 'not_found'. See this module's docstring: this extraction has not been "
                 "validated against a broad, live filing sample."),
    }
...
def main(argv=None):
    ...
    if not symbols:
        LOG.warn("No symbols configured in operating_kpi_universe_media.json, nothing to collect")
        return 1
